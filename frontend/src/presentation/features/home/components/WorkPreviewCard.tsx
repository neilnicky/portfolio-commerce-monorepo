...
export function WorkPreviewCard({ vm }: { vm: WorkVm }) {
  return (
    <Link
      href={vm.href ?? "#"}
      className="group block cursor-pointer transition-transform duration-500 ease-cinematic hover:-translate-y-1.5"
    >
      <MediaFrame src={vm.imageSrc} alt={vm.imageAlt} aspect="aspect-[16/9]" duration="duration-1000" className="mb-6" />
      <div className="flex items-start justify-between">
        <div>
          <h3 className="mb-1 font-display text-headline-sm uppercase tracking-tight transition-colors duration-300 group-hover:text-primary">
            {vm.title}
          </h3>
        ...
        <Icon
          name="north_east"
          className="translate-y-1 opacity-0 transition-all duration-300 ease-cinematic group-hover:translate-y-0 group-hover:opacity-100"
        />
      </div>
    </Link>
  );
